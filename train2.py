...
def train(total_steps=10000, ckpt_path=None, save_path="deepseek_checkpoint.pt", 
    ...
    loader = DataLoaderLite(B=16, T=256, vocab_size=config.vocab_size)
    ...
    print(f"  Effective batch size: {loader.B * grad_accum_steps}")
    ...
    for step in range(start_step, total_steps):
        ...
        t1 = time.time()
        # Calculate throughput based on TOTAL tokens processed (all micro-steps)
        total_tokens = loader.B * loader.T * grad_accum_steps
        tok_per_sec = total_tokens / (t1 - t0)
        dt = t1 - t0

        if step % log_interval == 0:
            # Estimate MFU (model flops utilization)
            mfu = model.estimate_mfu(loader.B * grad_accum_steps, dt)
            print(f"step {step:5d} | loss {loss_accum.item():.4f} | lr {lr:.6f} | tok/s {tok_per_sec:8.1f} | MFU {mfu*100:.2f}%")
        ...
        if step % 1000 == 0 and step > 0:
            ...
            save_checkpoint(model, optimizer, step, loss_accum.item(), ckpt_name, lr_config)

    print(f"\n{'='*70}")
    print(f"Training Complete!")
    print(f"Final loss: {loss_accum.item():.4f}")
    print(f"{'='*70}\n")
    save_checkpoint(model, optimizer, total_steps, loss_accum.item(), save_path, lr_config)
